...
-- Block
START TRANSACTION;
CREATE OR REPLACE FUNCTION block_count() RETURNS trigger
    LANGUAGE plpgsql AS
$$
BEGIN
...
END;
$$;
DROP TRIGGER IF EXISTS block_count_mod ON block;
CREATE CONSTRAINT TRIGGER block_count_mod
    AFTER INSERT OR DELETE
    ON block
    DEFERRABLE INITIALLY DEFERRED
    FOR EACH ROW
EXECUTE PROCEDURE block_count();
-- TRUNCATE triggers must be FOR EACH STATEMENT
DROP TRIGGER IF EXISTS block_count_trunc ON block;
CREATE TRIGGER block_count_trunc
    AFTER TRUNCATE
    ON block
    FOR EACH STATEMENT
EXECUTE PROCEDURE block_count();
...
UPDATE total
SET count = (SELECT count(*) FROM block)
WHERE name = 'blocks';
COMMIT;

-- Extrinsics
START TRANSACTION;
CREATE OR REPLACE FUNCTION extrinsic_count() RETURNS trigger
    LANGUAGE plpgsql AS
$$
BEGIN
    IF TG_OP = 'INSERT' THEN
...
END;
$$;
DROP TRIGGER IF EXISTS extrinsic_count_mod ON extrinsic;
CREATE CONSTRAINT TRIGGER extrinsic_count_mod
    AFTER INSERT OR DELETE
    ON extrinsic
    DEFERRABLE INITIALLY DEFERRED
    FOR EACH ROW
EXECUTE PROCEDURE extrinsic_count();
-- TRUNCATE triggers must be FOR EACH STATEMENT
DROP TRIGGER IF EXISTS extrinsic_count_trunc ON extrinsic;
CREATE TRIGGER extrinsic_count_trunc
    AFTER TRUNCATE
    ON extrinsic
    FOR EACH STATEMENT
EXECUTE PROCEDURE extrinsic_count();
-- initialize the counter table
UPDATE total
SET count = (SELECT count(*) FROM extrinsic)
WHERE name = 'extrinsics';
COMMIT;


-- Events
START TRANSACTION;
CREATE OR REPLACE FUNCTION event_count() RETURNS trigger
    LANGUAGE plpgsql AS
$$
BEGIN
    IF TG_OP = 'INSERT' THEN
...
END;
$$;
DROP TRIGGER IF EXISTS event_count_mod ON event;
CREATE CONSTRAINT TRIGGER event_count_mod
    AFTER INSERT OR DELETE
    ON event
    DEFERRABLE INITIALLY DEFERRED
    FOR EACH ROW
EXECUTE PROCEDURE event_count();
-- TRUNCATE triggers must be FOR EACH STATEMENT
DROP TRIGGER IF EXISTS event_count_trunc ON event;
CREATE TRIGGER event_count_trunc
    AFTER TRUNCATE
    ON event
    FOR EACH STATEMENT
EXECUTE PROCEDURE event_count();
-- initialize the counter table
UPDATE total
SET count = (SELECT count(*) FROM event)
WHERE name = 'events';
COMMIT;

-- Transfers
START TRANSACTION;
CREATE OR REPLACE FUNCTION transfer_count() RETURNS trigger
    LANGUAGE plpgsql AS
$$
BEGIN
...
END;
$$;
DROP TRIGGER IF EXISTS transfer_count_mod ON extrinsic;
CREATE CONSTRAINT TRIGGER transfer_count_mod
    AFTER INSERT OR DELETE
    ON extrinsic
    DEFERRABLE INITIALLY DEFERRED
    FOR EACH ROW
EXECUTE PROCEDURE transfer_count();
-- TRUNCATE triggers must be FOR EACH STATEMENT
DROP TRIGGER IF EXISTS transfer_count_trunc ON extrinsic;
CREATE TRIGGER transfer_count_trunc
    AFTER TRUNCATE
    ON extrinsic
    FOR EACH STATEMENT
EXECUTE PROCEDURE transfer_count();
-- initialize the counter table
UPDATE total
SET count = (SELECT count(*) FROM extrinsic WHERE method IN ('transfer', 'transferKeepAlive'))
WHERE name = 'transfers';
COMMIT;

INSERT INTO total (name, count)
VALUES ('transactions_fees', 0)
ON CONFLICT DO NOTHING;
...
BEGIN
    IF NEW.is_signed = 'true' THEN
        IF TG_OP = 'INSERT' THEN
            UPDATE total
            SET count = count + CAST(NEW.fee_info::json ->> 'partialFee' AS INTEGER)
...
END;
$$;
DROP TRIGGER IF EXISTS transactions_fees_mod ON extrinsic;
CREATE CONSTRAINT TRIGGER transactions_fees_mod
    AFTER INSERT OR DELETE
    ON extrinsic
    DEFERRABLE INITIALLY DEFERRED
    FOR EACH ROW
EXECUTE PROCEDURE transactions_fees();
-- TRUNCATE triggers must be FOR EACH STATEMENT
DROP TRIGGER IF EXISTS transactions_fees_trunc ON extrinsic;
CREATE TRIGGER transactions_fees_trunc
    AFTER TRUNCATE
    ON extrinsic
    FOR EACH STATEMENT
EXECUTE PROCEDURE transactions_fees();
-- initialize the counter table
UPDATE total
SET count = COALESCE((SELECT SUM(CAST(fee_info::json ->> 'partialFee' AS BIGINT))
                      FROM extrinsic
...
CREATE OR REPLACE VIEW signed_extrinsics_per_day_view AS
SELECT *
FROM (SELECT "when"::date
      FROM generate_series((select to_timestamp(timestamp)::date from extrinsic where block_number = '1'), now(),
                           interval '1 day') "when") d
         LEFT JOIN (
...
CREATE OR REPLACE VIEW signed_extrinsics_per_month_view AS
