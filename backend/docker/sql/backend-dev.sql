GRANT ALL PRIVILEGES ON DATABASE dev_cerestats TO dev_cerestats;
...
GRANT ALL PRIVILEGES ON TABLE block TO dev_cerestats;
GRANT ALL PRIVILEGES ON TABLE harvest_error TO dev_cerestats;
GRANT ALL PRIVILEGES ON TABLE event TO dev_cerestats;
GRANT ALL PRIVILEGES ON TABLE extrinsic TO dev_cerestats;
GRANT ALL PRIVILEGES ON TABLE ranking TO dev_cerestats;

GRANT ALL PRIVILEGES ON TABLE era_vrc_score TO dev_cerestats;
GRANT ALL PRIVILEGES ON TABLE era_commission TO dev_cerestats;
GRANT ALL PRIVILEGES ON TABLE era_commission_avg TO dev_cerestats;
GRANT ALL PRIVILEGES ON TABLE era_self_stake TO dev_cerestats;
GRANT ALL PRIVILEGES ON TABLE era_self_stake_avg TO dev_cerestats;
GRANT ALL PRIVILEGES ON TABLE era_relative_performance TO dev_cerestats;
GRANT ALL PRIVILEGES ON TABLE era_relative_performance_avg TO dev_cerestats;
GRANT ALL PRIVILEGES ON TABLE era_points TO dev_cerestats;
GRANT ALL PRIVILEGES ON TABLE era_points_avg TO dev_cerestats;
...
GRANT ALL PRIVILEGES ON TABLE faucet TO dev_cerestats;
...
-- Signed Transactions
START TRANSACTION;
CREATE FUNCTION signed_transaction_count() RETURNS trigger LANGUAGE plpgsql AS
$$BEGIN
  IF NEW.is_signed = true THEN
    IF TG_OP = 'INSERT' THEN
      UPDATE total SET count = count + 1 WHERE name = 'signed_transactions';
      RETURN NEW;
    ELSIF TG_OP = 'DELETE' THEN
      UPDATE total SET count = count - 1 WHERE name = 'signed_transactions';
      RETURN OLD;
    ELSE
      UPDATE total SET count = 0 WHERE name = 'signed_transactions';
      RETURN NULL;
    END IF;
  END IF;
  RETURN NULL;
END;$$;
CREATE CONSTRAINT TRIGGER signed_transaction_count_mod
  AFTER INSERT OR DELETE ON extrinsic
  DEFERRABLE INITIALLY DEFERRED
  FOR EACH ROW EXECUTE PROCEDURE signed_transaction_count();
-- TRUNCATE triggers must be FOR EACH STATEMENT
CREATE TRIGGER signed_transaction_count_trunc AFTER TRUNCATE ON extrinsic
  FOR EACH STATEMENT EXECUTE PROCEDURE signed_transaction_count();
-- initialize the counter table
UPDATE total SET count = (SELECT count(*) FROM extrinsic WHERE is_signed = true) WHERE name = 'signed_transactions';
COMMIT;