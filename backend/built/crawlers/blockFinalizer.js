...
const crawler = async () => {
    logger.info(loggerOptions, `Delaying block finalizer start for ${config.startDelay / 1000}s`);
    ...
    await api.rpc.chain.subscribeFinalizedHeads(async (blockHeader) => {
        ...
        for (let blockToUpdate = trackedBlock + 1; blockToUpdate <= blockNumber; blockToUpdate++) {
            await updateFinalizedBlock(config, api, client, blockToUpdate, loggerOptions);
        }
        trackedBlock = blockNumber;
    });
};
