...
import { getLastEraInDb, getThousandValidators, getAddressCreation, parseIdentity, getClusterInfo, getCommissionHistory, getCommissionRating, getPayoutRating, insertEraValidatorStats, insertEraValidatorStatsAvg, insertRankingValidator, addNewFeaturedValidator, } from '../lib/staking';
    ...
    try {
        ...
        for (const validator of validators) {
            ...
            if (validator.identity.parent) {
                ...
                stashAddressesCreation[stashParentAddress] = await getAddressCreation(client, stashParentAddress, loggerOptions);
            }
        }
        ...
        if (parseInt(activeEra, 10) - 1 > parseInt(lastEraInDb, 10)) {
            ...
            await Promise.all(ranking.map((validator) => insertEraValidatorStats(client, validator, activeEra, loggerOptions)));
            logger.debug(loggerOptions, 'Storing era stats averages in db...');
            await Promise.all(eraIndexes.map((eraIndex) => insertEraValidatorStatsAvg(client, eraIndex, loggerOptions)));
        }
        else {
        ...
        await Promise.all(ranking.map((validator) => insertRankingValidator(client, validator, blockHeight, startTime, loggerOptions)));
        logger.debug(loggerOptions, 'Cleaning old data');
        await dbQuery(client, `DELETE FROM ranking WHERE block_height != '${blockHeight}';`, loggerOptions);
        // featured validator
