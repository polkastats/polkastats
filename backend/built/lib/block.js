...
export const healthCheck = async (config, client, loggerOptions) => {
...
export const storeMetadata = async (client, blockNumber, blockHash, specName, specVersion, timestamp, loggerOptions) => {
    let metadata;
    try {
...
export const harvestBlock = async (config, api, client, blockNumber, loggerOptions) => {
    var _a;
    const startTime = new Date().getTime();
    try {
      ...
      ;`;
        try {
            ...
            const endTime = new Date().getTime();
            logger.info(loggerOptions, `Added block #${blockNumber} (${shortHash(blockHash.toString())}) in ${((endTime - startTime) /
        ...
        if (runtimeUpgrade) {
            ...
            await storeMetadata(client, blockNumber, blockHash.toString(), specName.toString(), specVersion.toNumber(), timestamp, loggerOptions);
        }
        await Promise.all([
            // Store block extrinsics
            processExtrinsics(api, apiAt, client, blockNumber, blockHash, block.extrinsics, blockEvents, timestamp, loggerOptions),
            // Store module events
            processEvents(client, blockNumber, parseInt(activeEra.toString()), blockEvents, block.extrinsics, timestamp, loggerOptions),
            // Store block logs
            processLogs(client, blockNumber, block.header.digest.logs, timestamp, loggerOptions),
        ]);
    }
    catch (error) {
        logger.error(loggerOptions, `Error adding block #${blockNumber}: ${error}`);
...
export const harvestBlocksSeq = async (config, api, client, startBlock, endBlock, loggerOptions) => {
...
export const harvestBlocks = async (config, api, client, startBlock, endBlock, loggerOptions) => {
    const reverseOrder = true;
    const blocks = reverseOrder
        ? reverseRange(startBlock, endBlock, 1)
        : range(startBlock, endBlock, 1);
    ...
    for (const chunk of chunks) {
        ...
        await Promise.all(chunk.map((blockNumber) => harvestBlock(config, api, client, blockNumber, loggerOptions)));
...
export const updateFinalizedBlock = async (config, api, client, blockNumber, loggerOptions) => {
    const startTime = new Date().getTime();
    try {
        const blockHash = await api.rpc.chain.getBlockHash(blockNumber);
      ...
      WHERE block_number = $7`;
        ...
        const endTime = new Date().getTime();
        logger.info(loggerOptions, `Updated finalized block #${blockNumber} (${shortHash(blockHash.toString())}) in ${((endTime - startTime) / 1000).toFixed(config.statsPrecision)}s`);
    }
    catch (error) {
        logger.error(loggerOptions, `Error updating finalized block #${blockNumber}: ${error}`);
        Sentry.captureException(error);
    }
};
