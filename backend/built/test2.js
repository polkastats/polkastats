...
const chunkSize = 10;
const harvestBlock = async (api, blockNumber) => {
    try {
...
const harvestBlocks = async (api, startBlock, endBlock) => {
    const blocks = range(startBlock, endBlock, 1);
    const chunks = chunker(blocks, chunkSize);
    for (const chunk of chunks) {
        await Promise.all(chunk.map((blockNumber) => (0, exports.harvestBlock)(api, blockNumber)));
    }
};
