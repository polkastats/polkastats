...
module.exports = {
  ...
  updateTotals: async (client, loggerOptions) => {
    const sql = `
        ...
        UPDATE total SET count = (SELECT count(block_number) FROM extrinsic) WHERE name = 'extrinsics';
        UPDATE total SET count = (SELECT count(block_number) FROM extrinsic WHERE section = 'balances' and method = 'transfer' ) WHERE name = 'transfers';
        UPDATE total SET count = (SELECT count(block_number) FROM event) WHERE name = 'events';
      `;
    try {
      await client.query(sql);
    } catch (error) {
