...
export default {
  ...
  data() {
    return {
	  chart: new ChartLineOptions('era', undefined, 'Secondary'),
    //   chartOptions: {
    //     responsive: true,
    //     legend: {
    //       display: false,
  ...
  computed: {
    chartData() {
		this.chart.setData(this.commissionHistory.map(({ commission }) => commission));
		this.chart.setLabels(this.commissionHistory.map(({ era }) => era));
