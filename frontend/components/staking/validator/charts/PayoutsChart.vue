<template>
  <BarChart :data="chartData" :options="chart.options" :height="200" />
</template>
<script>
import BarChart from '@/components/charts/BarChart.js'
import { config } from '@/frontend.config.js'
import { ChartBarOptions } from '@/components/charts/settings/chartbar';
...
export default {
  ...
  data() {
    return {
	  chart: new ChartBarOptions('era', undefined, 'Secondary'),
      config,
    //   chartOptions: {
    //     responsive: true,
    //     legend: {
  ...
  computed: {
    chartData() {
		...
		this.chart.setLabels(this.payoutHistory.map(({ era }) => era));
