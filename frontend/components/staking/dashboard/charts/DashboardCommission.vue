...
export default {
  ...
  data() {
    return {
	  chart: new ChartLineOptions('era', this.$t('components.dashboard_commission.avg_commission')),
      config,
    //   chartOptions: {
    //     responsive: true,
  ...
  apollo: {
    $subscribe: {
      ...
      network_commission_avg: {
		...
		this.chart.setData(this.rows.map((row) => row.commission_avg));
		this.chart.setLabels(this.eras);
